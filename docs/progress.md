...
- Active task: P1-008 — PostgreSQL local infrastructure
- Latest clean commit: `e616fac9593c236dd7684113d5b3285c0653f5cb`
...
| P1-004 | Repository hierarchy | active | `e616fac` | Layout/ownership check begun; no empty placeholder tree created | Complete remaining TS packages, service roots, scripts, CI, security, and Terraform-owned paths. |
| P1-005 | pnpm/TypeScript workspace | active | `e616fac` | pnpm 11.24 lock/install plus initial strict build/typecheck/test passed on Node 24.20 | Add the remaining Phase 1 packages, services, and web app to the workspace. |
| P1-006 | uv/Python workspace | complete | pending infrastructure commit | uv 0.12.7 sync, all four packages imported/built; Ruff, Pyrefly, and pytest passed on Python 3.14.7 | Preserve this runtime boundary when Or-on packages are imported later. |
...
| P1-008 | PostgreSQL local infrastructure | active | pending infrastructure commit | Compose definition and health check added; Docker CLI/daemon unavailable on this host | Complete daemon-backed PostgreSQL 18.6 health proof when Docker is installed. |
| P1-009 | Alembic foundation | active | pending infrastructure commit | Exactly one head and deterministic offline PostgreSQL SQL generation passed | Run upgrade/current/check against the Compose PostgreSQL instance. |
...
| P1-012 | Python service foundations | complete | pending infrastructure commit | Control API lifecycle/liveness/readiness tests pass; dispatcher and voice entrypoints import with dependency-safe/default-off lifecycle | Later phases integrate retained Or-on packages into these boundaries. |
...
| P1-016 | Observability foundation | active | `e616fac` | Shared structured JSON logging and recursive secret redaction tests pass in Python and TypeScript | Add correlation propagation and convention documentation during service/web integration. |
...
Commit PostgreSQL/Alembic and Python service foundations, then implement explicit
contracts, TypeScript services, the design-system package, and the unified web shell.
