...
| P4-006 | WhatsApp webhook and provider simulator | active | pending | Default-safe development simulator enforces auth/CSRF and canonical provider-message dedupe; exact raw-body HMAC-SHA256 verification and defensive Meta text-envelope parsing pass unit tests | Persist verified envelopes durably before acknowledgement; keep real-provider activation disabled. |
