...
| RenikudPlus / RenikudPlus weights | Or-on auto-downloads `notmax123/RenikudPlus` | Exact package/model licenses were not recorded in-repo. Resolve before redistribution/deployment image inclusion. |
| ECAPA gender model weights | Or-on downloads `JaesungHuh/voice-gender-classifier`; architecture is ported from Jpost/TaoRuijie code | Exact code/model licenses and attribution need resolution. High-priority provenance gap. |
