...
DB-role and cross-tenant RLS execution is `BLOCKED` without isolated PostgreSQL.
...
| Members, roles and API keys | `/users`, `/roles`, settings member/key APIs | Owner/admin per assignment matrix; API keys scoped, returned once and revocable | **PASS** — auth, management, settings and UI tests | **BLOCKED** — runtime revocation/RLS requires PostgreSQL |
...
| Classifications, notes, custom fields and service locations | Contact dossier APIs; `packages/ts/crm/src/contacts.ts` and `field-service.ts` | CRM read/write and tenant scope; these customer-file features remain available when field service is disabled | **PASS** — route/domain tests; location update/archive PostgreSQL cases authored | **BLOCKED** — online FK/RLS execution skipped |
...
| WhatsApp AI ownership, reply and human takeover | Messaging worker AI jobs and orchestration handoffs | Published eligible tenant agent; current feature/ownership rechecked before send | **PASS** — generation/provider delay/takeover and worker retry simulations | **BLOCKED** — real recipient delivery and in-flight provider cancellation |
...
| Voice sessions, campaigns, controls, recordings and usage | `/voice*`, voice APIs → control/dispatcher/agent/session stores | Voice read/operate; provider flags; pinned versions/knowledge | **PASS** — 1,064-test Python suite covers Hebrew, latency logic, barge-in, grounding, sessions and artifacts | **BLOCKED** — no two-way audio, native-Hebrew listener or actual provider receipt in this execution |
| Cross-channel WhatsApp callback | Durable callback job → dispatcher → linked conversation/customer/session | Explicit request/consent, eligible agent/flow, tenant and ownership recheck | **PASS** — mocked provider/job retry/idempotency tests | **BLOCKED** — no bounded real callback |
| Field-service entitlement and activation | `/settings`, field-service settings/archive APIs; typed registry → entitlement/configuration tables | Super-admin grants availability; owner/admin activates dependencies; disabled by default | **PASS** — API/worker/feature tests, disabled queued-work cancellation | **BLOCKED** — migration/RLS not executed online |
| Technician profiles and case lifecycle | `/field-service`, case/technician APIs; CRM field service → service schema | Read/operate/manage split; active linked technician role validation; feature required | **PASS** — transition graph, link/edit/deactivate, paging and API regressions | **BLOCKED** — online trigger/FK/concurrency execution skipped |
| Manual scheduling, reassignment and conflict prevention | Appointment APIs/UI → transactional scheduling service | Field-service operate/manage; active technician; tenant timezone | **PASS** — deterministic domain tests and PostgreSQL cases authored | **BLOCKED** — exclusion/conflict queries not run on PostgreSQL; external booking is not claimed |
...
| Health, deployment and recovery | `/system/health`, CI, Compose, Caddy and deploy/backup scripts | Protected diagnostics; immutable image/source identity; deploy authority external | **PASS** — source/archive/backup/drain/rollback test suite and shell syntax | **BLOCKED** — Docker daemon, isolated restore, off-host backup and authorized DEV release |
...
The complete journey is **BLOCKED**, not failed or passed: its source-level
segments have deterministic coverage, but there is no isolated PostgreSQL
runtime, authenticated DEV QA identity, controlled provider manifest or actual
device/audio environment. A fresh tenant → contact → WhatsApp → AI → handoff →
task/calendar → service visit/report → finance journey must be executed after the
prerequisites in [Release assessment](release-assessment.md) are met. No segment
was simulated and then mislabeled as a deployed/provider pass.
