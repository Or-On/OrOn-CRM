...
| Release image preload | Default Compose pulls omit services behind profiles, so a clean release host could lack the one-shot migrator image at revision validation | Deployment now pulls the release, workers and voice profiles before any digest inspection or active-release interruption. The release regression asserts that preload precedes image inspection. |
