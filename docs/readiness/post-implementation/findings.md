...
| Host edge readiness | The VM could not reliably connect to its own public address even though all candidate containers were healthy, causing the public-hostname probe to roll back a valid release | The in-host probe validates Caddy on `127.0.0.1` while retaining the configured hostname for SNI, certificate and redirect checks. The GitHub runner separately verifies the actual public HTTP and HTTPS route. Origin syntax is validated before use. |
