...
| Release image preload | Aggregate Compose pulls omitted the profiled one-shot migrator even when profiles were enabled, so a clean release host lacked that image at revision validation | Deployment explicitly pulls each of the five validated immutable application references, then the two base images, before any digest inspection or active-release interruption. The release regression asserts all keys and the pull-before-inspect order. |
| Deployer bootstrap identity | CI invoked the previously installed host deployer, so a fix packaged inside a candidate archive could not repair the rollout that needed it | CI separately transfers the deployer from the checked-out commit, binds it to a SHA-256 output, verifies it on the host and executes that exact file. The archive remains independently checksum-bound and the successful deploy installs the same script for recovery/service use. |
| Host edge readiness | The hardened extractor normalized the bind-mounted Caddyfile to root-only `0640` while Caddy runs as unprivileged UID `65532`; the replacement container therefore opened no listener even though Compose briefly reported it started | The non-secret Caddyfile is normalized to `0644`, with a Linux permission regression. The in-host probe validates loopback with the real hostname for SNI/certificate/redirect behavior, emits bounded Caddy/listener diagnostics before rollback, and the GitHub runner separately verifies the public route. |
