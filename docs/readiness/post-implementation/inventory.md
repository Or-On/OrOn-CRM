...
| Inbox media and field-service intake | Canonical WhatsApp media → `whatsapp.media.retrieve` (legacy `field_service.whatsapp_media.retrieve` jobs remain accepted); Field Service intake → `field_service.intake.extract` | Media mirroring follows channel/provider safety independently; case-attachment projection happens after Inbox persistence and rechecks the live feature/intake switches |
