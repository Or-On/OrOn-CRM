...
| Identity, tenants, invitations, roles, billing, OAuth | [Security inventory](security-inventory.md); [billing/auth concurrency evidence](../../.artifacts/readiness/candidate/docs/readiness/billing-security-fixes.md); [risk register](findings.md) | Inventory complete; selected state/financial/authorization controls verified-local | Complete all-role API/browser matrix, invitation tenant destination and public-account safeguards; separately authorized provider acceptance |
| WhatsApp, voice, orchestration, jobs | [Worker inventory](workers-inventory.md); [worker repairs](../../.artifacts/readiness/candidate/docs/readiness/worker-fixes.md) | Inventory complete; nine bounded worker findings repaired/tested; voice/automatic-call blockers remain | Close WRK-009–013, prove restart/reconciliation, then authorize controlled provider E2E |
