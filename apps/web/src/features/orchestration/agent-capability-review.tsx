  ...
  return (
    <>
      ...
      <section className="agent-review" aria-label={copy.actionsTitle}>
        ...
        {(agent.incompatibleFeatures?.length ?? 0) === 0 ? null : (
          <div role="alert" className="agent-review__warnings">
            <h5>{copy.incompatibleTitle}</h5>
            <p>
              {copy.incompatible(agent.incompatibleFeatures?.join(", ") ?? "")}
            </p>
          </div>
        )}
