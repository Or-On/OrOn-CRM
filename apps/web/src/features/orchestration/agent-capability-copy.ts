const en = {
  ...
  incompatibleTitle: "Published version is blocked",
  incompatible: (features: string) =>
    `This version uses disabled tenant modules: ${features}. Enable them or publish a compatible revision before rebinding.`,
  blocking: {
    lead_schema_missing:
...
const he: Copy = {
  ...
  incompatibleTitle: "הגרסה שפורסמה חסומה",
  incompatible: (features) =>
    `הגרסה משתמשת במודולים מושבתים של סביבת העבודה: ${features}. יש להפעיל אותם או לפרסם גרסה תואמת לפני שיוך מחדש.`,
  blocking: {
