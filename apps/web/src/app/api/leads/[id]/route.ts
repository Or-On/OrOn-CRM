...
export async function GET(
  ...
  try {
    ...
    const detail = await withCurrentTenant("crm:read", (sql) =>
      getLeadDetail(sql, uuid(id, "Lead")),
    );
    if (detail === undefined)
...
export async function PATCH(
  ...
  try {
    ...
    const lead = await withCurrentTenant("crm:write", (sql, session) =>
      updateLeadForOperator(sql, session.userId, uuid(id, "Lead"), update),
    );
