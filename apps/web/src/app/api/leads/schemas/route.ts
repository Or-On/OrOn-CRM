...
export async function GET() {
  try {
    return NextResponse.json({
      schemas: await withCurrentTenant("crm:read", async (sql) => {
        await requireTenantFeature(sql, "leads");
        return listLeadFieldSchemas(sql);
      }),
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const schema = await withCurrentTenant(
      "flows:manage",
      async (sql, session) => {
        await requireTenantFeature(sql, "leads");
        return createLeadFieldSchema(sql, session.userId, {
