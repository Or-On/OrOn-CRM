...
import { ControlApiClient, type AudioPreviewRequest } from "@or-on/api-client";
  ...
  try {
    ...
    if (
      ...
      return privateResponse(
        NextResponse.json(
          { error: "Invalid audio preview request" },
          { status: 400 },
        ),
      );
    }
    // Fresh membership/session check finishes before any paid network request.
    ...
    const command: AudioPreviewRequest = {
      agent_id: id,
      version_id: body.versionId,
      text: body.text.trim(),
      confirmed: true,
    };
    const result = await client.previewAgentAudio(command);
    if (!result.ok) {
      const status = [401, 403, 404, 429, 503, 504].includes(result.status)
        ? result.status
        : 503;
      return privateResponse(
        NextResponse.json({ error: "Audio preview unavailable" }, { status }),
      );
    }
    return privateResponse(NextResponse.json({ preview: result.data }));
  } catch (error) {
