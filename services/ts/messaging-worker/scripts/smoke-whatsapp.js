...
async function main() {
    ...
    if (recipient === undefined ||
        ...
        throw new Error("provide --recipient and exactly one of --text or --template");
    const config = loadConfig(process.env, {
        requireWhatsApp: true,
        service: "whatsapp-smoke",
    });
    if (!config.enableRealWhatsApp)
        throw new Error("refusing: ENABLE_REAL_WHATSAPP must explicitly equal true");
    let delivery;
    if (text !== undefined) {
    ...
    const confirmation = await prompt.question(`REAL Meta delivery (${delivery.kind}) to ***${suffix}. Type SEND_REAL_WHATSAPP to continue: `);
    prompt.close();
    ...
    stdout.write(`Meta accepted the message. ID suffix: ${result.messageId.slice(-8)}\n`);
}
main().catch((error) => {
    const reason = error instanceof Error ? error.message : "unknown smoke-test failure";
