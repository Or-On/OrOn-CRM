...
def initial_node_from_spec(
    ...
    *,
    ...
    try:
        ...
        if runtime_function_factories:
            fallback_configs = {str(fallback["name"]): fallback}
            fallback["functions"] = [
                *fallback.get("functions", []),
                *(
                    factory(str(fallback["name"]), fallback_configs)
                    for factory in runtime_function_factories
                ),
            ]
        return fallback
